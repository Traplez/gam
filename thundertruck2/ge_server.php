<?php
 ...
 if(!in_array ($status, $gamer_groups))
	...
	{
	global $user_id;
	   $bank=get_bank($user_id,'thundertruck2',$type);
	    return $bank;
	}
	...
	setcookie("intro_thundertruck2", $_POST['show_intro'],time() + 86400);
	...
	if ($action != 'error')
	{
		...
		elseif (($action == 'finish') && !isset($_SESSION['thundertruck2_win']))
		{
			$action = 'error';
	...
	if ( $action == "state" )
	{
		...
		if (isset($_SESSION['thundertruck2_win']))
			unset($_SESSION['thundertruck2_win']);
		if (isset($_SESSION['thundertruck2_freeGameCount']))
			unset($_SESSION['thundertruck2_freeGameCount']);
		if (isset($_SESSION['thundertruck2_d']))
			unset($_SESSION['thundertruck2_d']);
	...
	if ( $action == "spin" )
	{
	    ...
	    $rowb9 = get_game_settings('thundertruck2');
	    $proc4 = $rowb9['g_proc'];
	    $allbet23 = $allbet / 100 * $proc4;
	   change_bank($user_id,'thundertruck2',$allbet23,"spin");
	$pr30_shans=rand(1,3);
...
if ( ($rnd_bonus == 1) && ($casbank < $psym[13][3]*$allbet) ){
	...
	///////////////////////////
		...
		if($rnd_bonus==1 ){
			...
			$_SESSION['thundertruck2_freeGameCount']=25;
...
for($j=0; $j<=2; $j++){
	...
	if($am>=120000 && $winall>0){
			...
			$bank_result=change_bank($user_id,'thundertruck2',$winall*-1,"spin");
	...
	$_SESSION['microbank']-=$winall44;
			$_SESSION['thundertruck2_win'] = $winall;
		}
...
$lin[19]=array(1,6,9,12,13);		
	...
	$_SESSION['thundertruck2_mpl']=1;		
	
	$cMpl=1;
	
	$rndWild=rand(1,3);
	
	if($_POST['gameMode']=="odin" && $rndWild==1){
...
for($j=0; $j<=2; $j++){
			...
			$currentMpl=$_SESSION['thundertruck2_mpl'];
	        ...
	        for ( $k = 1; $k <= 14; ++$k )
	        {
	            ${ "win".$k } = $bet * $map_win[$k]*$cMpl;
				$winall+=${ "win".$k };
	        }
			
			
	      
		  if($winall>0 && $_SESSION['thundertruck2_freeMode']==1){
			
		$_SESSION['thundertruck2_mpl']++;	
			
           if($_SESSION['thundertruck2_mpl']>5){
	      $_SESSION['thundertruck2_mpl']=5;
           }
			
		  }else  if($winall<=0 && $_SESSION['thundertruck2_freeMode']==1){
			 $_SESSION['thundertruck2_mpl']=2;  
		  }
	...
	if($am>=120000 && $winall>0){
			...
			$bank_result=change_bank($user_id,'thundertruck2',$winall*-1,"bonus");
...
$anim_sym_str=implode("|",$anim_sym);
		ge_serv_show_str( "result=ok&ravMpl=$cMpl&anim_sym_str=|$anim_sym_str&info=|$sym1|$sym2|$sym3|$sym4|$sym5|$sym6|$sym7|$sym8|$sym9|$sym10|$sym11|$sym12|$sym13|$sym14|$sym15|". 
    "$bet|$line|$allbet|$winall|0|$allbet|$winall|0|1|0&id=$user_array[login]&balance=$user_balance&scatter_win=$scatter_win&curMpl=".$_SESSION['thundertruck2_mpl']);
...
/*   Риск-игра  */
	if ( $action == "double" )
	{
		...
		{
		    $d = isset($_SESSION['thundertruck2_d']) ? $_SESSION['thundertruck2_d'] + 1 : 1;
			$_SESSION['thundertruck2_d'] = $d;
			$winall = $_SESSION['thundertruck2_win'];
		...
		$_SESSION['microbank']+=$winall44;
			...
			$row_bon=get_game_settings('thundertruck2');
			...
			if ( $winall2 > 0 )
			{
				...
				$_SESSION['thundertruck2_win']+=$_SESSION['thundertruck2_win2'];
			...
			$stat_txt = "thundertruck2_double";
			$winall44 = sprintf( "%01.2f", $winall2 );
            unset($_SESSION['thundertruck2_win2']);
			...
			$_SESSION['microbank']-=$winall44;
				$_SESSION['thundertruck2_win'] = $winall2;
			}
			
			if($double_bank_sum!=0){
			change_bank($user_id,'thundertruck2',$double_bank_sum,"double");
			}
